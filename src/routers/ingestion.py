...
@router.post("/vessels/")
async def ingest_vessel(vessel: Vessel, session: Session = Depends(get_session)):
    session.add(vessel)
    session.commit()
...
async def ingest_satellite(satellite: Satellite, session: Session = Depends(get_session)):
